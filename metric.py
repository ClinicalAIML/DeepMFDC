...
import sys
sys.path.append('../pADPClust')
#from pADPClust.utils import utils
from pAdpClust import AdpClust
from AdpConfig import AdpConfig
...
def inference(loader, model, device, view, data_size):
    ...
    #labels_vector = np.array(labels_vector).reshape(100)
...
def valid(model, device, dataset, view, data_size, class_num, eval_h=False):
    ...
    return acc, nmi, ari, pur, total_pred



def valid_(model, device, dataset, view, data_size, class_num, eval_h=False):
    test_loader = DataLoader(
            dataset,
            batch_size=data_size,
            shuffle=False,
